...
export default function QuizBuilderPage() {
  ...
  useEffect(() => {
    if (formId) {
      ...
      if (f) setForm(JSON.parse(JSON.stringify(f)))
    }
  }, [formId])

  if (!form) {
    return (
      <div className="min-h-screen bg-[#07090e] text-slate-300 flex items-center justify-center">
        <div className="flex items-center gap-2">
          <div className="w-4 h-4 rounded-full border-2 border-amber-500 border-t-transparent animate-spin" />
          <span>Carregando construtor InLegado...</span>
        </div>
      </div>
    )
  }
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                      Fluxo de Perguntas ({form.questions.length})
                    </label>
                    <button
                      type="button"
            ...
            {activeTab === 'theme' && (
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between p-4 rounded-xl bg-slate-900/60 border border-slate-800">
                  ...
                  <input
                    type="checkbox"
                    checked={form.theme.showProgressBar}
                    onChange={(e) => setForm({
                      ...form,
